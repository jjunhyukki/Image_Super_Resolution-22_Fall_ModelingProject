...
Github : https://github.com/HeaseoChung/EDSR-PyTorch
"""
...
class ResBlock(nn.Module):
    def __init__(self, n_feats, res_scale=1.0):
        ...
        m = []
        for i in range(2):
            m.append(nn.Conv2d(n_feats, n_feats,
                     kernel_size=3, bias=True, padding=3//2))
            if i == 0:
...
class EDSR(nn.Module):
    def __init__(self, scale_factor=2, num_channels=3, num_feats=64, num_blocks=16, res_scale=1.0):
        ...
        self.head = nn.Conv2d(num_channels, num_feats,
                              kernel_size=3, padding=3//2)
        body = [ResBlock(num_feats, res_scale) for _ in range(num_blocks)]
        self.body = nn.Sequential(*body)
        self.tail = nn.Sequential(
            nn.Conv2d(num_feats, num_feats * (scale_factor ** 2),
                      kernel_size=3, stride=1, padding=1),
            nn.PixelShuffle(scale_factor),
            nn.ReLU(True),
            nn.Conv2d(num_feats, num_channels,
                      kernel_size=3, stride=1, padding=1),
        )
